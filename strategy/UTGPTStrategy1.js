// =============================================================================
// UTGPTStrategy1 — UT Bot (Key=2, ATR=1) + Supertrend(10,3) + chatGpt EMA
//
// BUY conditions (Supertrend must ALWAYS be bullish):
//   1) Supertrend bullish first (or same candle) + UT Bot flips bullish.
//      No chatGpt needed. ST cannot come AFTER UT Bot.
//   2) UT Bot or chatGpt — either triggers first, then the second one
//      triggers while inside Supertrend.
//
// SELL:
//   If UT Bot flips to SELL on the latest candle, SELL immediately (takes
...
function utGptStrategy1(candles) {
  ...
  // UT Bot 1 pre-condition (Key=2, ATR=1)
  ...
  if (stBullish && ut1.flippedBuy) {
    return {
      signal: "BUY",
      reason: stFlippedBuy
        ? "UT Bot + ST flip together (K2/ATR1)"
        : "UT Bot flip (K2/ATR1) inside ST bullish",
  ...
  // ── SELL priority: UT Bot SELL flip on the latest candle ──
  if (ut1.flippedSell) {
    return {
      signal: "SELL",
      reason: "UT Bot sell flip (Key=2, ATR=1)",
      ema10: ema10_now, ema20: ema20_now,
      utBot1Pos: ut1.pos, utBot1Trail: ut1.trail,
      stDirection: stLast, supertrend: stLine[stLine.length - 1]
    };
  ...
  // ── BUY Path 2a: UT Bot already bullish + chatGpt triggers + inside ST ──
  if (ut1Bullish && stBullish && chatGptJustTriggered) {
    return {
      signal: "BUY",
      reason: "chatGpt trigger + UT Bot bullish (K2/ATR1) + ST bullish",
      ema10: ema10_now, ema20: ema20_now,
      utBot1Pos: ut1.pos, utBot1Trail: ut1.trail,
      stDirection: stLast, supertrend: stLine[stLine.length - 1]
    };
  ...
  if (chatGptBullishState && stBullish && ut1.flippedBuy) {
    return {
      signal: "BUY",
      reason: "UT Bot flip (K2/ATR1) + chatGpt bullish + ST bullish",
      ema10: ema10_now, ema20: ema20_now,
      utBot1Pos: ut1.pos, utBot1Trail: ut1.trail,
      stDirection: stLast, supertrend: stLine[stLine.length - 1]
    };
