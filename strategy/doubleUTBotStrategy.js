...
// BUY:  Supertrend bullish + (UT Bot 1 BUY OR UT Bot 2 BUY)
...
function doubleUTBotStrategy(candles) {
  ...
  for (let i = 1; i < N; i++) {
    ...
    // ── BUY: Supertrend bullish + (UT Bot 1 OR UT Bot 2) BUY signal ──
    if (!inPosition && stBullish && (buy1 || buy2)) {
      inPosition = true;
      sig = "BUY"; trade = "ENTRY";
      reason = buy1 ? "UT Bot 1 buy + ST bullish" : "UT Bot 2 buy + ST bullish";
    }
